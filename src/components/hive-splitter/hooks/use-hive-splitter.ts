...
export default function useHiveSplitter(
  ...
  const { addOnMouseDown, addOnTouchStart, addOnClick, touch } =
    useSplitterTouch(splitterRef, handleEvent, panes, panesCount, horizontal);

  const formatSize = (size: number | string): number => {
    if (size !== null && `${size}`.match(/px$/)) {
      const divider = horizontal
        ? splitterRef.value?.offsetHeight!
        : splitterRef.value?.offsetWidth!;
  ...
  const updatePaneStyle = function (pane: Pane): void {
    let size: number | string | null = indexedPanes.value[pane.id].size;

    pane.updateStyle({
      [horizontal ? "height" : "width"]: `${
        size !== null ? formatSize(size) : size
      }%`,
    });
  };

  const readjustSizes = async (
    spaceToAllocate: number,
    ungrowablePanes: string[],
    unshrinkablePanes: string[]
  ) => {
    let equalSpaceToAllocate: number;
    if (spaceToAllocate > 0) {
      equalSpaceToAllocate =
        spaceToAllocate / (panesCount.value - ungrowablePanes.length);
    } else {
      equalSpaceToAllocate =
        spaceToAllocate / (panesCount.value - unshrinkablePanes.length);
    }

    panes.value.forEach((pane, i) => {
      if (spaceToAllocate > 0 && !ungrowablePanes.includes(pane.id)) {
        const newPaneSize = Math.max(
          Math.min(Number(pane.size) + equalSpaceToAllocate, pane.max),
          pane.min
        );
    ...
    if (Math.abs(spaceToAllocate) > 0.1) {
      await nextTick(() => {
        if (isReady.value) {
          console.warn(
            "Splitter: Could not resize panes correctly due to their constraints."
          );
        }
      });
    }
        ...
        if (pane.size === null) {
          pane.size = formatSize(
            Math.max(
              Math.min(
                spaceToAllocate /
                  (panesCount.value - panesWithDefinedSizesCount),
                pane.max
              ),
              pane.min
            )
  ...
  const redoSplitters = () => {
    const children = Array.from(
      (splitterRef.value as HTMLElement).children
    ) as HTMLElement[];
  ...
  const onPaneAdd = async (pane: VueComponent) => {
    ...
    const min = formatSize(getFormattedSize(pane.minSize));
    const max = formatSize(getFormattedSize(pane.maxSize));
    const newPane = {
      id: pane.id,
      index,
