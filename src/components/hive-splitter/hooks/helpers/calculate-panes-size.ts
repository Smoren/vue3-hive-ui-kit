...
const sumPrevPanesSize = (panes: Ref<Pane[]>, splitterIndex: number) => panes.value.reduce(
    (total, pane, i) => total + (i < splitterIndex ? Number(pane.size) : 0),
    0
  );

const sumNextPanesSize = (panes: Ref<Pane[]>, splitterIndex: number) => panes.value.reduce(
    (total, pane, i) => total + (i > splitterIndex + 1 ? Number(pane.size) : 0),
    0
  );
...
const getCurrentDragPercentage = (
  ...
  const containerSize = (splitterRef.value as HTMLElement)[
    horizontal ? 'clientHeight' : 'clientWidth'
  ];
...
const findPrevExpandedPane = (panes: Ref<Pane[]>, splitterIndex: number) => {
  const pane = [...panes.value]
    .reverse()
    .find((p) => p.index < splitterIndex && p.size && p.size > p.min);
  return pane || ({} as Pane);
};

const findNextExpandedPane = (panes: Ref<Pane[]>, splitterIndex: number) => {
  const pane = panes.value.find(
    (p) => p.index > splitterIndex + 1 && p.size && p.size > p.min
  );
  return pane || ({} as Pane);
};
...
export default function calculatePanesSize({
  ...
  const dragPercentage = Math.max(
    Math.min(
      getCurrentDragPercentage(horizontal, splitterRef, cursorPosition),
      maxDrag
    ),
    minDrag
  );
  ...
  const paneAfterMaxReached =
    paneAfter.max < 100 &&
    dragPercentage <=
      100 - (paneAfter.max + sumNextPanesSize(panes, splitterIndex + 1));
    ...
    if (paneBeforeMaxReached) {
      ...
      paneBefore.size = Math.max(
        100 -
          paneAfter.max -
          sums.prevPanesSize -
          sumNextPanesSize(panes, splitterIndex + 1),
        0
      );
